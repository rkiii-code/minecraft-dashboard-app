...
export function AdminMetricsPage() {
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      <div>
        <h1 className="page-title">Admin / Metrics</h1>
        <p className="page-subtitle">scoreboard objective の追加・無効化。バックエンド接続まではモック操作です。</p>
      </div>

      <Card title="追加" subtitle="objective は英小文字とアンダースコアのみ想定">
        <form onSubmit={handleAdd} className="grid" style={{ gap: 10 }}>
          ...
          <div className="hint">実 API につないだら POST /api/admin/metrics を呼び出す想定です。</div>
        </form>
      </Card>
      ...
      <Card title="メトリクス一覧" subtitle={loading ? '読み込み中...' : `${rows.length} 件`}>
        ...
        {rows.length === 0 && <div className="muted">データなし</div>}
      </Card>
    </div>
  );
}
