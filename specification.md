...
- フロントエンドは `AuthContext` で認証状態をグローバル管理。
- `ProtectedRoute` コンポーネントで認証必須ルートを保護。
- ロール制御: admin のみ `/admin/*` ルートにアクセス可能。

### 3.4.1 モックログイン（開発用）

`VITE_USE_MOCK=true` 時に有効な動作確認用アカウント:

| ユーザー名 | パスワード | ロール | 説明                         |
| ---------- | ---------- | ------ | ---------------------------- |
| `admin`    | `admin123` | admin  | 管理者（メトリクス管理可能） |
| `user`     | `user123`  | user   | 一般ユーザー（閲覧のみ）     |
| `sun5un`   | `password` | admin  | テスト管理者                 |
| `umi_bird` | `password` | user   | テストユーザー               |

モックユーザーは `src/data/mock.ts` の `mockUsers` 配列で定義。
